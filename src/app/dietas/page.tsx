...
import { Plus, Loader2, Search, UtensilsCrossed, Trash2, X, Eye } from "lucide-react"
...
export default function DietasPage() {
  ...
  const [viewingDieta, setViewingDieta] = useState<any>(null)
  const [editingDieta, setEditingDieta] = useState<any>(null)
  const [deletingDieta, setDeletingDieta] = useState<any>(null)
  ...
  const loadDietas = async () => {
    try {
      ...
      toast.error(error.message || 'Erro ao carregar dietas')
    } finally {
      setLoading(false)
    }
  }
  ...
  const loadPreMisturas = async () => {
    try {
      const response = await fetch('/api/pre-misturas?ativas=true')
      const data = await response.json()
      ...
      console.error('Erro ao carregar pré-misturas:', error)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validIngredientes.length < 1) {
      toast.error('A dieta deve ter pelo menos 1 ingrediente')
      return
    }
    ...
    try {
      ...
      const url = editingDieta 
        ? `/api/dietas/${editingDieta.id}`
        : '/api/dietas'
      
      const method = editingDieta ? 'PUT' : 'POST'
  ...
  const handleDelete = async () => {
    if (!deletingDieta) return

    try {
      setSubmitting(true)

      const response = await fetch(`/api/dietas/${deletingDieta.id}`, {
        method: 'DELETE',
      })
      ...
      toast.success(data.message)
      setDeleteOpen(false)
      setDeletingDieta(null)
      loadDietas()
    } catch (error: any) {
      toast.error(error.message || 'Erro ao excluir dieta')
    } finally {
      setSubmitting(false)
    }
  }

  const openEditDialog = async (dieta: any) => {
    try {
      const response = await fetch(`/api/dietas/${dieta.id}`)
      const data = await response.json()
      ...
      setIngredientes(data.dieta.ingredientes.map((ing: any) => ({
        tipo: ing.tipo,
        insumo_id: ing.insumo_id || undefined,
        pre_mistura_id: ing.pre_mistura_id || undefined,
      ...
      toast.error(error.message || 'Erro ao carregar dieta')
    }
  }

  const openViewDialog = async (dieta: any) => {
    try {
      const response = await fetch(`/api/dietas/${dieta.id}`)
      const data = await response.json()
  ...
  const addIngrediente = () => {
    setIngredientes([...ingredientes, { tipo: 'insumo', percentual_mistura: 0, percentual_ms: 0, valor_unitario_kg: 0 }])
  }
  ...
  const updateIngrediente = (index: number, field: keyof Ingrediente, value: any) => {
    ...
    // Limpar IDs ao mudar tipo
    if (field === 'tipo') {
      newIngredientes[index].insumo_id = undefined
      newIngredientes[index].pre_mistura_id = undefined
    }
      ...
      <SidebarInset>
          ...
          <div className="flex items-center gap-2 px-4">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage>Dietas</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="h-24 text-center">
                        <Empty>
                          ...
                          <EmptyContent>
                            <EmptyTitle>Nenhuma dieta encontrada</EmptyTitle>
                            <EmptyDescription>
                              {searchTerm ? 'Tente ajustar sua busca' : 'Comece criando sua primeira dieta'}
                            </EmptyDescription>
                          </EmptyContent>
                        </Empty>
                      </TableCell>
                    ...
                    filteredDietas.map((dieta) => (
                      <TableRow key={dieta.id}>
                        ...
                        <TableCell className="text-right font-mono">
                          {Number(dieta.percentual_ms_total).toFixed(2)}%
                        </TableCell>
                        <TableCell className="text-right">
                          {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(dieta.custo_kg)}
                        </TableCell>
                        <TableCell className="text-center">
                          <Badge variant={dieta.ativo ? "default" : "secondary"}>
                            {dieta.ativo ? "Sim" : "Não"}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
        ...
        <Dialog open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) resetForm() }}>
          <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingDieta ? 'Editar Dieta' : 'Nova Dieta'}</DialogTitle>
              <DialogDescription>
                {editingDieta ? 'Atualize os dados da dieta' : 'Crie uma dieta com insumos e/ou pré-misturas'}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Dieta de Terminação"
                    required
                  />
                </div>
                  ...
                  {ingredientes.map((ing, index) => (
                    <div key={index} className="grid gap-3 p-4 border rounded-lg relative">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select
                          value={ing.tipo}
                          onValueChange={(value) => updateIngrediente(index, 'tipo', value)}
                          required
                        >
                          <SelectTrigger>
                            <SelectValue />
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="pre_mistura">Pré-Mistura</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>

                      <div className="grid gap-2">
                        <Label>{ing.tipo === 'insumo' ? 'Insumo' : 'Pré-Mistura'} *</Label>
                        {ing.tipo === 'insumo' ? (
                          <Select
                            value={ing.insumo_id || ''}
                          ...
                          <Select
                            value={ing.pre_mistura_id || ''}
                            onValueChange={(value) => updateIngrediente(index, 'pre_mistura_id', value)}
                            required
                          >
                            <SelectTrigger>
                              <SelectValue placeholder="Selecione a pré-mistura" />
                            </SelectTrigger>
                            <SelectContent>
                              {preMisturas.map((pm) => (
        ...
        <Dialog open={viewOpen} onOpenChange={setViewOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              <DialogTitle>{viewingDieta?.nome}</DialogTitle>
              <DialogDescription>
                Detalhes da dieta
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold">{Number(viewingDieta.percentual_ms_total).toFixed(2)}%</p>
                  </div>
                  <div>
                    <Label className="text-sm text-muted-foreground">Custo/kg</Label>
                    <p className="text-lg font-semibold">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(viewingDieta.custo_kg)}
                    </p>
                  </div>
                  <div>
                    <Label className="text-sm text-muted-foreground">Status</Label>
                    <p className="text-lg font-semibold">{viewingDieta.ativo ? 'Ativo' : 'Inativo'}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {viewingDieta.ingredientes?.map((ing: any, index: number) => (
                        <TableRow key={index}>
                          <TableCell>
                            <Badge variant="outline">
                              {ing.tipo === 'insumo' ? 'Insumo' : 'Pré-Mistura'}
                            </Badge>
                          </TableCell>
        ...
        <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Tem certeza que deseja excluir a dieta <strong>{deletingDieta?.nome}</strong>?
